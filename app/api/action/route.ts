...
`;
  ...
  if(totalHigherBalance <= 5000){
    return NextResponse.json({ message: `User has ${totalHigherBalance.toFixed(2)} [Fish🐠]` }, { status: 200 });
  }
  else if(totalHigherBalance >=5001 && totalHigherBalance <= 30000){ 
  return NextResponse.json({ message: `User has ${totalHigherBalance.toFixed(2)} [Shrimp🦐]` }, { status: 200 });
  }else if(totalHigherBalance >=30001 && totalHigherBalance <= 60000){
    return NextResponse.json({ message: `User has ${totalHigherBalance.toFixed(2)} [Dolphine🐬]` }, { status: 200 });
  }else if(totalHigherBalance >=60001 && totalHigherBalance <= 100000){
    return NextResponse.json({ message: `User has ${totalHigherBalance.toFixed(2)} [Shark🦈]` }, { status: 200 });
  }else if(totalHigherBalance >=100001){
    return NextResponse.json({ message: `User has ${totalHigherBalance.toFixed(2)} [Whale🐋]` }, { status: 200 });
  }
}
